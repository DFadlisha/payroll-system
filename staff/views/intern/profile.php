...
<div class="page-header">
    <h1><i class="bi bi-person me-2"></i>Intern Profile</h1>
</div>
...
<?php if ($user): ?>
    <div class="row g-4">
        ...
        <div class="col-lg-4">
            <?php
            // Role specific settings for Intern
            $themeColor = 'warning';
            $roleLabel = 'Intern';
            $icon = 'bi-mortarboard';
            ?>
            ...
            <div class="card glass-card border-0 animate-fade-in h-100">
                ...
                <div class="card-header border-0 bg-transparent text-center py-5 position-relative overflow-hidden">
                    <div class="position-absolute top-0 start-0 w-100 h-100" 
                         style="background: linear-gradient(135deg, rgba(245, 158, 11, 0.1) 0%, rgba(251, 191, 36, 0.1) 100%); z-index: 0;"></div>
                    ...
                    <div class="position-relative z-1">
                        ...
                        <span class="badge bg-warning-soft text-warning rounded-pill px-4 py-2 border border-warning-subtle">
                ...
                <div class="card-body p-4">
                    <div class="d-flex flex-column gap-3">
                        <div class="d-flex justify-content-between align-items-center p-3 rounded-3 bg-white bg-opacity-50 border border-light">
                            <span class="text-muted small text-uppercase fw-bold"><i class="bi bi-building me-2 text-warning"></i>Company</span>
                            <span class="fw-semibold text-end text-dark"><?= htmlspecialchars($user['company_name'] ?? 'N/A') ?></span>
                        </div>
                        <div class="d-flex justify-content-between align-items-center p-3 rounded-3 bg-white bg-opacity-50 border border-light">
                            <span class="text-muted small text-uppercase fw-bold"><i class="bi bi-person-workspace me-2 text-warning"></i>Type</span>
                            <span class="badge bg-secondary-soft text-secondary border"><?= getEmploymentTypeName($user['employment_type']) ?></span>
                        </div>
                        <div class="d-flex justify-content-between align-items-center p-3 rounded-3 bg-white bg-opacity-50 border border-light">
                            <span class="text-muted small text-uppercase fw-bold"><i class="bi bi-cash me-2 text-warning"></i>Basic Salary</span>
                            <span class="fw-bold text-success font-monospace fs-5"><?= formatMoney($user['basic_salary']) ?></span>
                        </div>

                        <?php if (!empty($user['internship_months'])): ?>
                            <div class="d-flex justify-content-between align-items-center p-3 rounded-3 bg-warning bg-opacity-10 border border-warning-subtle">
                                <span class="text-muted small text-uppercase fw-bold"><i class="bi bi-hourglass-split me-2 text-warning"></i>Duration</span>
                                <span class="fw-bold text-dark"><?= $user['internship_months'] ?> Months</span>
                            </div>
                        <?php endif; ?>

                        <?php if (!empty($user['ic_number'])): ?>
                            <div class="d-flex justify-content-between align-items-center p-3 rounded-3 bg-white bg-opacity-50 border border-light">
                                <span class="text-muted small text-uppercase fw-bold"><i class="bi bi-card-heading me-2 text-warning"></i>IC No.</span>
        ...
        <div class="col-lg-8">
            <div class="card glass-card border-0 animate-fade-in" style="animation-delay: 0.1s;">
                <div class="card-header bg-transparent border-bottom border-light py-4 ps-4">
                    <h5 class="mb-0 fw-bold"><i class="bi bi-pencil-square me-2 text-warning"></i>Update Information</h5>
                </div>
                <div class="card-body p-4">
                    <form method="POST" class="needs-validation" novalidate>
                        <input type="hidden" name="action" value="update_profile">
                        <!-- Personal Info -->
                        <h6 class="text-warning text-uppercase small fw-bold mb-4 ps-1 border-start border-4 border-warning ps-2">Personal Information</h6>
                        ...
                        <!-- Bank Info -->
                        <h6 class="text-warning text-uppercase small fw-bold mb-4 ps-1 border-start border-4 border-warning ps-2">Bank Information</h6>
                        <div class="row g-4 mb-5">
                            <div class="col-md-6">
